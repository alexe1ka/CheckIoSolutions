...
# Первый - это карта подключение электроснабжения, представленна в виде списка (list) подключений.
# Каждое соеденение представляет список (list) из двух узлов. Город или электростанция могут быть узлами.
# Каждый город или электростанция является уникальной строкой (string).
# Второй аргумент - это словарь (dict) где ключом является номер электростанции,
# а значением ключа - мощность электростанции.
# Выходные данные: Список (list) строк. Каждая строка является именем обесточенного города.
